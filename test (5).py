...
def check_formalities(transitions, holding_time):
    ...
    methods = ['get_states', 'set_state', 'current_state',
    ...
    for i in range(NUM_CHANGES):
        ...
        for j in range(hd):
            if weather_sim.current_state() != last_state:
                print(
                    f'Error: State {last_state} changed before holding time {hd} to {weather_sim.current_state()}!')
                return False
...
def run_test(transitions, holding_time, avg, tolerance):
    ...
    if any(list(map(lambda x: x[0] > x[1], zip(diff, tolerance)))):
        ...
        print(
            f'Higher range: {list(map(lambda x: round(x[0]+x[1],2), zip(avg,tolerance)))}')
        print(f'Your result: {freq}')
        print(
            f'Lower range: {list(map(lambda x: round(x[0]-x[1],2), zip(avg,tolerance)))}')
    else:
